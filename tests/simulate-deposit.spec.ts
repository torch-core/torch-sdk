...
describe('Simulate & Deposit Testcases', () => {
  ...
  const depositImpactTriTON = async (
    ...
    // Send deposit
    const response = await torchSDK.simulateDeposit(depositFluctuateParams);
    const sendDepositFluctuateArgs = await response.getDepositPayload(sender, { blockNumber });
  ...
  describe('Deposit in TriTON Pool', () => {
    ...
    it('should deposit TON, tsTON and stTON to triTON pool', async () => {
      ...
      // Send deposit
      const response = await torchSDK.simulateDeposit(depositParams);
      const sendDepositArgs = await response.getDepositPayload(sender, { blockNumber });
