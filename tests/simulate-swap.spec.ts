...
describe('Simulate and Swap Testcases', () => {
  ...
  describe('Swap in TriTON Pool', () => {
    it('should swap tsTON to stTON by routes', async () => {
      ...
      // Send swap
      const response = await torchSDK.simulateSwap(swapExactInParams);

      const sendExactInArgs = await response.getSwapPayload(sender, { blockNumber });
    ...
    it('should swap tsTON to stTON (ExactIn + ExactOut)', async () => {
      ...
      // Send swap
      const responseOut = await torchSDK.simulateSwap(swapExactOutParams);
      const sendExactOutArgs = await responseOut.getSwapPayload(sender, { blockNumber });
    ...
    it('should refund tsTON to sender when min amount out is not met (tsTON -> stTON ExactIn)', async () => {
      ...
      // Send swap
      const response = await torchSDK.simulateSwap(swapParams);
      const sendArgs = await response.getSwapPayload(sender, { blockNumber });
      await send(sendArgs);
      ...
      // Send swap
      const responseSlippage = await torchSDK.simulateSwap(swapSlipageParams);
      const sendSlipageArgs = await responseSlippage.getSwapPayload(sender, { blockNumber });
  ...
  describe('Cross pool swap', () => {
    ...
    describe('Swap and Withdraw', () => {
      ...
      it('should refund hTON to sender when slippage is not met (hTON -> stTON ExactIn)', async () => {
        ...
        const responseFluctuate = await torchSDK.simulateSwap(swapFluctuateParams);
        const sendFluctuateArgs = await responseFluctuate.getSwapPayload(sender, { blockNumber });
